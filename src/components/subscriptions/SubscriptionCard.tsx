...
export default function SubscriptionCard({
    ...
    return (
        <div className={`p-4 rounded-xl border shadow-sm ${sub.estado === 'ACTIVO' ? 'bg-[#0a0a0a] border-white/6' : 'bg-[#0a0a0a] border-white/6 opacity-75'}`}>
            ...
            <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                    ...
                    <label className="text-xs text-white/35 font-semibold uppercase">WhatsApp</label>
                    <input
                        className="block w-full font-mono text-slate-200 font-medium bg-transparent focus:bg-[#111] focus:ring-1 focus:ring-[#25D366] rounded px-1 -ml-1"
                ...
                <button
                    ...
                    className={`ml-2 px-2 py-1 rounded-lg text-[10px] font-bold border transition-colors flex items-center gap-1 ${sub.estado === 'ACTIVO' ? 'bg-emerald-900/30 text-emerald-400 border-emerald-800' : 'bg-[#111] text-white/35 border-white/8'}`}
                >
            ...
            {/* Email */}
            <div className="mb-3">
                <label className="text-xs text-white/35 font-semibold uppercase">
                    {(sub.servicio || 'CANVA') === 'CANVA' ? 'Correo' : 'Correo de la Cuenta'}
                </label>
                <div className="flex items-center gap-2">
                    <input
                        className="block w-full text-sm text-white/65 bg-transparent focus:bg-[#111] focus:ring-1 focus:ring-[#25D366] rounded px-1 -ml-1 truncate"
                        defaultValue={sub.correo}
                        onBlur={(e) => onUpdate(sub.id, 'correo', e.target.value)}
                    />
                    <button
                        onClick={() => onCopy(sub.correo)}
                        className="p-2 text-white/40 hover:text-[#4ade80] hover:bg-[#111] rounded-lg transition-colors"
                        title="Copiar correo"
                    >
                        <Copy size={20} />
                    </button>
            ...
            {sub.password && (
                <div className="mb-4 p-3 bg-[#111]/60 rounded-lg border border-white/8/50">
                    <label className="text-xs text-white/35 font-semibold uppercase">Contraseña</label>
                    <div className="flex items-center gap-2 mt-1">
                        <span className="flex-1 text-sm font-mono text-white/65">
                            {showPassword ? sub.password : '••••••••'}
                        </span>
                        <button
                            onClick={() => setShowPassword(p => !p)}
                            className="p-1.5 text-white/40 hover:text-slate-200 transition-colors"
                        >
                            {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                        </button>
                        <button
                            onClick={() => onCopy(sub.password!)}
                            className="p-1.5 text-white/40 hover:text-[#4ade80] transition-colors"
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <label className="text-xs text-white/35 font-semibold uppercase">Vencimiento</label>
                    <input
                ...
                <div>
                    <label className="text-xs text-white/35 font-semibold uppercase">Equipo</label>
                    <input
                        className="block w-full text-sm text-white/65 bg-transparent focus:bg-[#111] focus:ring-1 focus:ring-[#25D366] rounded px-1 -ml-1"
            ...
            {/* Actions */}
            <div className="pt-4 border-t border-white/6 flex justify-end gap-2">
                ...
                <button onClick={() => onDelete(sub.id)} className="px-4 py-2 bg-[#111] text-white/35 border border-white/8 rounded-lg hover:text-red-400 hover:border-red-900/50 hover:bg-red-900/20 transition-colors">
                    <Trash2 size={18} />
                </button>
            </div>
        </div>
