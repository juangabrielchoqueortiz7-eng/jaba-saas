...
export default function FlowsPage() {
    ...
    const handleSaveAutomation = async (form: typeof EMPTY_JOB) => {
        ...
        // Si el target es suscripciones por vencer, sincronizar el umbral de días
        // en subscription_settings para que el recordatorio del sistema use el mismo valor
        if (form.target_type === 'subscriptions_expiring') {
            await supabase.from('subscription_settings').upsert(
                { user_id: user.id, notify_days_before: form.trigger_days_before },
                { onConflict: 'user_id' }
            )
        }
